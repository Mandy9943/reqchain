...
pub fn eval(input: &str, scope: &Scope) -> Result<String, ExprError> {
    ...
    if p.i != p.s.len() {
        return Err(ExprError::Syntax {
            message: format!("unexpected trailing input at byte {}", p.i),
        });
    }
    Ok(value)
}
...
impl<'a, 'b> Parser<'a, 'b> {
    ...
    fn varref(&mut self) -> Result<String, ExprError> {
        ...
        if !rest.starts_with("{{") {
            return Err(ExprError::Syntax {
                message: "expected a variable reference".into(),
            });
        }
        let end = rest.find("}}").ok_or(ExprError::Syntax {
    ...
    fn funcall(&mut self) -> Result<String, ExprError> {
        ...
        if name.is_empty() {
            return Err(ExprError::Syntax {
                message: format!("expected a function name at byte {start}"),
            });
        }
        self.skip_ws();
        if self.i >= self.s.len() || self.s[self.i] != b'(' {
...
fn apply(name: &str, args: &[String]) -> Result<String, ExprError> {
    match name {
        "md5" => {
            ...
            Ok(format!(
                "{:x}",
                md5::Md5::digest(one(name, args)?.as_bytes())
            ))
        }
        "sha1" => {
            use sha1::Digest;
            Ok(format!(
                "{:x}",
                sha1::Sha1::digest(one(name, args)?.as_bytes())
            ))
        }
        "sha256" => {
            use sha2::Digest;
            Ok(format!(
                "{:x}",
                sha2::Sha256::digest(one(name, args)?.as_bytes())
            ))
        }
        "base64" => {
            Ok(base64::engine::general_purpose::STANDARD.encode(one(name, args)?.as_bytes()))
        }
        "now" => Ok(chrono::Local::now()
            .format(&to_chrono_format(one(name, args)?))
            .to_string()),
...
fn to_chrono_format(fmt: &str) -> String {
    ...
    'outer: while !rest.is_empty() {
        ...
        if ch == '%' {
            out.push('%')
        } // escape a literal percent for chrono
        out.push(ch);
